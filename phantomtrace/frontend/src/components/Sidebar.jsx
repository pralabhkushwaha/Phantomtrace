...
import {
  LayoutDashboard, Mail, Globe2, Link2, FileSearch, ShieldCheck,
      ...
      <aside className={clsx(
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-dark-700">
          ...
          <div>
            ...
            <div className="text-xs text-cyber-400 font-mono tracking-widest">TRACE</div>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 space-y-0.5">
          {NAV_ITEMS.map(({ to, icon: Icon, label }) => (
            ...
            >
              {({ isActive }) => (
                <>
                  <Icon className={clsx('w-4 h-4 shrink-0', isActive ? 'text-cyber-400' : 'text-gray-500 group-hover:text-gray-400')} />
                  <span className="flex-1">{label}</span>
                  {isActive && <ChevronRight className="w-3 h-3 text-cyber-500" />}
                </>
              )}
            </NavLink>
          ))}
        ...
        <div className="px-5 py-4 border-t border-dark-700">
          <p className="text-[10px] text-dark-400 font-mono">
            PhantomTrace v1.0 · Forensics Platform
          </p>
        </div>
      </aside>
    </>
